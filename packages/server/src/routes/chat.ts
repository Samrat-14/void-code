...
import { getToolContracts, modeSchema, type ModeType, type ToolContracts } from "@voidcode/shared";
import { isSupportedChatModel, resolveChatModel } from "../lib/models";
import type { AuthenticatedEnv } from "../middleware/require-auth";
import { requireCreditsBalance } from "../middleware/require-credits-balance";
import { db } from "@voidcode/database/client";
import { buildSystemPrompt } from "../system-prompt";
import type { Prisma } from "@voidcode/database";
...
type VoidCodeUIMessage = UIMessage<ChatMessageMetadata, never, InferUITools<ToolContracts>>;

const submitSchema = z.object({
  id: z.string(),
  messages: z
    .array(
      z.custom<VoidCodeUIMessage>((value) => {
...
function hasPendingToolCalls(message: VoidCodeUIMessage) {
...
const app = new Hono<AuthenticatedEnv>().post(
  ...
  async (c) => {
    ...
    return result.toUIMessageStreamResponse<VoidCodeUIMessage>({
      ...
      async onFinish(event) {
        ...
        try {
          ...
          console.log("Failed to ingest Polar AI usage for chat message", {
            error,
            sessionId: id,
