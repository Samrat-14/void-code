import type { ModeType } from "@voidcode/shared";
...
export function buildSystemPrompt({ mode }: SystemPromptParams): string {
  ...
  parts.push(`You are an expert software engineer working as a coding assistant inside a terminal application.
    ...
    The applictaion has two modes the user can switch between:
  ...
  if (mode === "PLAN") {
    parts.push(`
      ...
      - Present you analysis and a clear plan of action
      - Explain trade-offs and ask for clarification when needed`);
  } else {
    parts.push(`
      ## Mode: BUILD
      You are in build mode. Your job is to imlement changes directly.
