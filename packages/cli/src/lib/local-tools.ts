...
import { toolInputSchemas, Mode, type ModeType } from "@voidcode/shared";
import { readOnlyToolContracts, buildToolContracts } from "../../../shared/src/schemas";
...
export async function executeLocalTool(toolName: string, input: unknown, mode: ModeType) {
  if (mode === Mode.PLAN && !Object.keys(readOnlyToolContracts).includes(toolName)) {
  ...
  switch (toolName) {
    ...
    case "grep": {
      ...
      const proc = Bun.spawn(["grep", ...args], { stdout: "pipe", stderr: "pipe", cwd });
      ...
      // grep exits with 1 when no matches found - not as error
    ...
    case "editFile": {
      ...
      if (occurences > 1)
        throw new Error(
          `oldString is ambiguous - found ${occurences} matches. Provide more surrounding context to make it unique.`,
        );
