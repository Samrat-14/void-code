...
import { chatStreamEventSchema, type SupportedChatModelId } from "@voidcode/shared";
...
export function useChat(sessionId: string, initialMessages: Message[]) {
  ...
  const hasAutoResumeRedf = useRef(false);
  useEffect(() => {
    if (hasAutoResumeRedf.current) return;

    const last = initialMessages[initialMessages.length - 1];
    if (!last || last.role !== "user") return;

    hasAutoResumeRedf.current = true;
