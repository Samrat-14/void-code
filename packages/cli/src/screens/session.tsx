...
import { useChat, type ClientMessagePart, type Message } from "../hooks/use-chat";
import { messagePartsSchema, type SupportedChatModelId } from "@voidcode/shared";
import prettyMs from "pretty-ms";
import { MessageStatus } from "@voidcode/database/enums";
...
function SessionChat({ session }: { session: SessionData }) {
  ...
  // Let the user cancel a reply even before the first streaming chunk arrives
  useKeyboard((key) => {
  ...
  useEffect(() => {
    ...
    const fetchSession = async () => {
      try {
        ...
        toast.show({
          variant: "error",
          message: err instanceof Error ? err.message : "faled to load session",
        });
